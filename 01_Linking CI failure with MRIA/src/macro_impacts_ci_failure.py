...
def macroimpact_estimation(RP1,op_factor1, imp_flex1, dam_thres_ci, dam_thres_in):
    ...
    # dam_thres_ci = 0.8 # flood depth in meters
    ...
    results_printer( MRIA_RUN1_NOCI, MRIA_RUN2_NOCI, MRIA_RUN3_NOCI, MRIA_RUN5_NOCI, DATA, op_factor, imp_flex, solvername, extension = f'noci_{RP}_{k}')
...
OPs = [1.0, 1.01, 1.025]                    # Overproduction factor
IFs = [0,  0.25, 1]                         # Flexibility factor

dam_thres_cis = [0, 1, 5]
dam_thres_ins = [1, 1, 1]

eac_ci_df = pd.DataFrame()
eac_noci_df = pd.DataFrame()

for k in range(len(dam_thres_cis)):
    for i in range(len(OPs)):
        for j in range(len(RPs)):
            ...
            op = OPs[i]
            tf = IFs[i]
            dam_thres_ci = dam_thres_cis[k]
            dam_thres_in = dam_thres_ins[k]

            macroimpact_estimation(rp, op, tf, dam_thres_ci, dam_thres_in)
            
        eac_ci  = expected_annual_impact_calculation(op, tf, 'ci', RPs, k)
        eac_noci = expected_annual_impact_calculation(op, tf, 'noci', RPs, k)

        eac_ci_df.loc[i,k] = eac_ci
        eac_noci_df.loc[i,k] = eac_noci


eac_ci_df.to_excel('eac_ci_df.xlsx')
eac_noci_df.to_excel('eac_noci_df.xlsx')
