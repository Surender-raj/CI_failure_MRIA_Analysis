...
def ci_assignment(exposure, substations, p):
    ...
    for sr in range(len(exposure)):
        ...
        sorted_df['probxweight'] = sorted_df['probability']*sorted_df['voltage_kv']
...
def macroimpact_estimation(RP1,op_factor1, imp_flex1, osmid_list, k):
    ...
    # adapted substation depth made zero
    ci.loc[ci['osmid'].isin(osmid_list), 'depth'] = 0
    ...
    osm_industrial = ci_assignment(osm_industrial, ci, p)
    ...
    results_printer( MRIA_RUN1_CI, MRIA_RUN2_CI, MRIA_RUN3_CI, MRIA_RUN5_CI, DATA, op_factor, imp_flex, solvername, extension = f'ci_{RP}_{k}')

    # Damage and funcitonality analysis results
    ci.to_excel(os.path.join('results', f'ci_{op_factor}_{imp_flex}_{solvername}_{RP}_{k}.xlsx'))
    osm_industrial.to_excel(os.path.join('results', f'osm_industrial_{op_factor}_{imp_flex}_{solvername}_{RP}_{k}.xlsx'))
    prod_df.to_excel(os.path.join('results', f'prod_df_{op_factor}_{imp_flex}_{solvername}_{RP}_{k}.xlsx'))
...
# A function to calcualte the investment cost
def investment_cost_calculator(criteria, r , n, maintenance, sequence, k):
    osmids  = list(sequence[criteria].unique())
    osmids_toadapt = osmids[0:k]
    path = os.path.join(os.getcwd(), 'results')
    ci_dam = pd.read_excel(os.path.join(path, f'ci_{1.01}_{0.25}_mosek_{10000}_{0}.xlsx'), index_col = [0], header = [0])
    input_path = os.path.join(os.path.dirname(os.getcwd()), 'data')
    ci = gpd.read_parquet(os.path.join(input_path, 'substaions_nl33.parquet'))
    ...
    return investment, surface_area
...
## Some adaptation related parameters
r = 0.025                       # discount rate 2.5%
n = 15                          # design life  = 15 years
maintenance = 0.20              # lifetme maintenance cost as a proportion of initial cost

# Sequence file
sequence = pd.read_parquet('sequence.parquet')
criterias = list(sequence.columns)


for m in range(len(criterias)):
    criteria = criterias[m]
    #criteria = criterias
    ...
    for k in range(len(sequence)+1):
        osmids  = list(sequence[criteria].unique())
        osmids_toadapt = osmids[0:k]                    ## A list of osm id s to adpat for this iteration
        ...
        for j in range(len(RPs)):
            ...
            macroimpact_estimation(rp, op, tf, osmids_toadapt,  k)
            
        eac_ci   = expected_annual_impact_calculation(op, tf, 'ci', RPs, k)
        investment, area = investment_cost_calculator(criteria, r , n, maintenance, sequence, k)
        cba_results.loc[k, 'EAC'] = eac_ci
        cba_results.loc[k, 'INV'] = investment
        cba_results.loc[k, 'TC']  = eac_ci + investment
        cba_results.loc[k, 'SA'] = area

    cba_results.to_excel(f'CBA_results_{criteria}.xlsx')
