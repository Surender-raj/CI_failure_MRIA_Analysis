...
class MRIA_SUT(object):
    ...
    def create_Ddis(self, rat_dict):
        ...
        def D_bounds_dis(model, R, P):
            ...
            # The number 0.001 is added to avoid strict bounds
            return (0, rat_dict[R,P]+0.001 )
        
            #return (0, min(rat_dict[R,P] + (0.001 * rat_dict[R,P]) , rat_dict[R,P]+0.001) )  
    ...
    def create_disimp_limits(self, all_disimp, imp_flex, distance_dict):
    ...
    def create_disaster_data(self, disr_dict_sup, disr_dict_dem, op_factor, all_disimp, imp_flex, distance_dict, rat_dict):
        """
        ...
        self.create_Xdis()
        self.create_Ddis(rat_dict)
        self.create_disimp_limits(all_disimp, imp_flex, distance_dict)
    ...
    def run_impactmodel(self, solvername):
        ...
        def objective_base(model):
            return sum(self.Xdis[R, S] for R in model.R for S in model.S)  + sum(self.disimp[Rb, R, P]*2  for Rb in model.R for R in model.R for P in model.P)
        ...
        if solvername == 'mosek':
            ...
            results = solver.solve(model, options = {'dparam.intpnt_tol_path' : 0.1,
                                                     'iparam.intpnt_max_iterations': 1000}, tee=False)
            #results.write()
